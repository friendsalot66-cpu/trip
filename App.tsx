...
export const App: React.FC = () => {
  ...
  const handleImportFromText = async () => {
    ...
    try {
        ...
        alert("Failed to parse itinerary. Please try again or use manual mode.");
    } finally {
  ...
  const loadTaipeiDemo = async () => {
      ...
      if (newTrip) {
          // Update title to custom chinese title
  ...
  const handleDeleteTrip = async (tripId: string) => {
      ...
      try {
          ...
          loadTrips(); // Reload to revert state
      }
  };
  ...
  const saveToHistory = () => {
      ...
      setHistory(prev => [...prev.slice(-9), currentDeepCopy]); // Keep last 10
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Helper to find which day contains a place
    ...
    // If hovering over a Place, use its day. If hovering over a Day Droppable, use that ID.
    const destDayId = findDayId(overId) || (days.find(d => d.dayId === overId)?.dayId);

    if (!sourceDayId || !destDayId) return;

    // 1. Reordering within the same day
    ...
    // 2. Moving between days
        ...
        setDays((prev) => {
            ...
            // Add to dest (at index of overId)
                    ...
                    <div>
                        ...
                        />
                         <p className="text-xs text-slate-500 mt-2">
                            Gemini AI will extract dates, locations, and times automatically.
                        </p>
                    </div>
    ...
    >
      ...
      {isProcessing && (
        <div className="fixed inset-0 z-[100] bg-black/50 flex flex-col items-center justify-center text-white backdrop-blur-sm">
            ...
            <p className="text-white/80 mt-2">Gemini is finding the best path for you.</p>
        </div>
      )}
